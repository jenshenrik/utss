...
public class PlayerControl : MonoBehaviour
{
    ...
    private void NextWeapon()
    {
        ...
        if (currentWewaponIndex > player.weaponList.Count)
        {
            currentWewaponIndex = 1;
        }

        SetWeaponByIndex(currentWewaponIndex);
    }
    ...
    private void PreviousWeapon()
    {
        ...
        if (currentWewaponIndex < 1)
        {
            currentWewaponIndex = player.weaponList.Count;
        }
    ...
    {
        // Add current weapon to temp list
        var tempWeaponList = new List<Weapon>();
        var currentWeapon = player.weaponList[currentWewaponIndex - 1];
        currentWeapon.weaponListPosition = 1;
        tempWeaponList.Add(currentWeapon);

        // Add remaining weapons to list
        int index = 2;
        foreach (var weapon in player.weaponList)
        {
            // skip current weapon which is already added
    ...
    private void ReloadWeaponInput()
    {
        ...
        // return if remaining ammo is less than clip capacity (ignore if ammo is infinite)
        ...
        // return if clip is already full
        ...
        {
            player.reloadWeaponEvent.CallReloadWeapon(player.activeWeapon.GetCurrentWeapon(), 0);
        }
    }
